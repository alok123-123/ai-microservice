...
async def _scrape_google_news_rss(
    ticker: str, max_articles: int = 15
) -> list[NewsArticle]:
...
async def scrape_news(ticker: str, max_articles: int = 15) -> list[NewsArticle]:
    """
    ...
    """
    articles = await _scrape_google_news_rss(ticker, max_articles)

    if len(articles) < 3:
        logger.info("Google News RSS yielded few results, trying Yahoo Finance…")
        yahoo_articles = await _scrape_yahoo_finance(ticker, max_articles)
